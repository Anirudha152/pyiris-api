    ...
    try:
        if scout_id == 'all':
            ...
            for i in list(self.config.scout_database.keys()):
                try:
                    ...
                    try:
                        ...
                        self.log.err('Scout does not exist in database!')
            if warning:
                return {"status": "warning", "message": "Successfully killed scouts. 1 or more scouts were already dead", "data": {"scout_database": self.config.scout_database}}
            else:
            ...
            try:
                ...
                self.log.err('Scout does not exist in database!')
            return {"status": "ok", "message": "Successfully killed scout", "data": {"scout_database": self.config.scout_database}}
