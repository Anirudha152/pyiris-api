...
def main(prompt = None):
    try:
        ...
        for i in comp_list:
            ...
            if interface == "GUI":
                if i == "windows/control/execute_python" or i == "linux/control/execute_python" or i == "windows/startup/sleep" or i == "linux/startup/sleep":
                    exec (i.replace('/', '_') + '.main("generate", "' + prompt + '")')
                else:
        ...
        if config.scout_values['Compile'][0] == 'True':
            ...
            if interface == "GUI":
                output = compiler.main(config.scout_values['Path'][0], prompt)
            elif interface == "CUI":
        ...
        if interface == "GUI":
            config.app.logger.error("[library/commands/generator_interface/generate] - File error (Is another process using a file of the same filepath?)")
        ...
        if interface == "GUI":
            config.app.logger.error("[library/commands/generator_interface/generate] - Error: " + str(e))
